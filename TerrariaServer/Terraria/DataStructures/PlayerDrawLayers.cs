...
public static class PlayerDrawLayers
{
	...
	public static void DrawPlayer_16_ArmorLongCoat(ref PlayerDrawSet drawinfo)
	{
		...
		if (matchingBodyExtension != -1)
		{
			...
			{
				DrawSittingLegs(ref drawinfo, TextureAssets.ArmorLeg[matchingBodyExtension].Value, drawinfo.colorArmorBody, drawinfo.cCoat, matchingBodyExtension);
			}
			else
			{
	...
	private static void DrawPlayer_21_Head_TheFace_Eyelid(ref PlayerDrawSet drawinfo)
	{
		...
		if (asset.IsLoaded)
		{
			...
			Rectangle value = asset.Frame(1, 3, 0, frameY);
			DrawData drawData = new DrawData(asset.Value, pos + new Vector2((int)(drawinfo.Position.X - Main.screenPosition.X - (float)(drawinfo.drawPlayer.bodyFrame.Width / 2) + (float)(drawinfo.drawPlayer.width / 2)), (int)(drawinfo.Position.Y - Main.screenPosition.Y + (float)drawinfo.drawPlayer.height - (float)drawinfo.drawPlayer.bodyFrame.Height + 4f)) + drawinfo.drawPlayer.headPosition + drawinfo.headVect + vector, value, color, drawinfo.drawPlayer.headRotation, drawinfo.headVect, 1f, drawinfo.playerEffect);
	...
	public static void DrawPlayer_22_FaceAcc(ref PlayerDrawSet drawinfo)
	{
		...
		if (drawinfo.drawPlayer.faceMask > 0 && drawinfo.drawPlayer.faceMask < ArmorIDs.Face.Count && (!isVelociraptor || ArmorIDs.Face.Sets.CanDrawOnVelociraptorMount[drawinfo.drawPlayer.faceMask]))
		{
		...
		if (drawinfo.drawPlayer.faceFlower > 0 && drawinfo.drawPlayer.faceFlower < ArmorIDs.Face.Count && (!isVelociraptor || ArmorIDs.Face.Sets.CanDrawOnVelociraptorMount[drawinfo.drawPlayer.faceFlower]))
		{
