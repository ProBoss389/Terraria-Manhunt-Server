...
public class StardustSky : CustomSky
{
	...
	{
		_planetTexture = Main.Assets.Request<Texture2D>("Images/Misc/StarDustSky/Planet", AssetRequestMode.ImmediateLoad);
		_bgTexture = Main.Assets.Request<Texture2D>("Images/Misc/StarDustSky/Background", AssetRequestMode.ImmediateLoad);
		_starTextures = new Asset<Texture2D>[2];
		for (int i = 0; i < _starTextures.Length; i++)
		{
			_starTextures[i] = Main.Assets.Request<Texture2D>("Images/Misc/StarDustSky/Star " + i, AssetRequestMode.ImmediateLoad);
		}
	}
