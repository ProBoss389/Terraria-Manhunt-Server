...
public class UIScrollbar : UIElement
{
	...
	public UIScrollbar(ColorTheme theme = ColorTheme.Blue)
	{
		...
		Width.Set(20f, 0f);
		MaxWidth.Set(20f, 0f);
		string assetName = "Images/UI/Scrollbar";
		if (_theme == ColorTheme.Cyan)
		{
			assetName = "Images/UI/Scrollbar2";
		}
		_texture = Main.Assets.Request<Texture2D>(assetName, AssetRequestMode.ImmediateLoad);
		_innerTexture = Main.Assets.Request<Texture2D>("Images/UI/ScrollbarInner", AssetRequestMode.ImmediateLoad);
		PaddingTop = 5f;
		PaddingBottom = 5f;
	}
