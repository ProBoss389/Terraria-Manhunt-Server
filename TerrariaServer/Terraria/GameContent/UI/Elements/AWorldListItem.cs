...
public abstract class AWorldListItem : UIPanel
{
	...
	protected UIElement GetIconElement()
	{
		if (_data.ZenithWorld)
		{
			Asset<Texture2D> asset = Main.Assets.Request<Texture2D>("Images/UI/IconEverythingAnimated", AssetRequestMode.ImmediateLoad);
	...
	private Asset<Texture2D> GetSeedIcon(string seed, bool hardmodeVariants = true, bool evilVariants = true)
	{
		...
		text += seed;
		return Main.Assets.Request<Texture2D>(text, AssetRequestMode.ImmediateLoad);
	}
}
