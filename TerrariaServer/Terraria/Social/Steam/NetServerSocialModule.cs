...
public class NetServerSocialModule : NetSocialModule
{
	...
	private void OnP2PSessionRequest(P2PSessionRequest_t result)
	{
		...
		else if (_acceptingClients && ((_mode & ServerMode.FriendsOfFriends) != ServerMode.None || SteamFriends.GetFriendRelationship(steamIDRemote) == EFriendRelationship.k_EFriendRelationshipFriend || !(steamIDRemote != SteamUser.GetSteamID())))
		{
			SteamNetworking.AcceptP2PSessionWithUser(steamIDRemote);
